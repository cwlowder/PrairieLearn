...
module.exports = {
    ...
    */
    ...
    _buildGradingJobStats(job) {
        const formatDiff = (start, end) => moment.utc(moment.duration(moment(end).diff(start)).asMilliseconds()).format('HH:mm:ss.SSS');
        return job ? {
            queueDuration: formatDiff(job.grading_requested_at, job.grading_received_at),
            gradingDuration: formatDiff(job.grading_received_at, job.graded_at),
            totalDuration: formatDiff(job.grading_requested_at, job.graded_at),
        } : null;
    },
